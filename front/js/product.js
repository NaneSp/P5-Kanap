...
addToCartBtn.addEventListener("click", function () {
  ...
  //Et si la quantité sélectionnée et inférieure à 1 OU supérieure à 100 --->soit une des 2 condition est true
    ...
    articleSelected.quantitySelected < 1 ||
    articleSelected.quantitySelected > 100
  ) {
    alert(
  ...
  //SINON
    ...
    /*1*/ if (articleLocalStorage == null) {
      ...
      alert(
        `Vous avez ajouté ${articleSelected.quantitySelected} ${articleSelected.nameSelected} de couleur ${articleSelected.colorSelected} à votre panier.`
      );
    ...
    /*2*/
      ...
      let articleFound = articleLocalStorage.find(
        ...
        (kanap) =>
          kanap.idSelected === articleSelected.idSelected &&
          kanap.colorSelected === articleSelected.colorSelected
      );
      ...
      /*A*/ if (articleFound) {
        ...
        let newQuantity =
          articleFound.quantitySelected + articleSelected.quantitySelected;
        articleFound.quantitySelected = newQuantity;
        localStorage.setItem("LSArticle", JSON.stringify(articleLocalStorage)); //setItem=ajout/stockage + json.stringify convertit un objet JS en données json
        alert(
          ` Vous avez ${articleSelected.quantitySelected} ${articleSelected.nameSelected} de couleur ${articleSelected.colorSelected} dans votre panier.`
        );
      }
